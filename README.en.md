...
> **Two-layer licensing** —
> ・**Source code**: MIT License (fork / modify / reuse freely within MIT scope)
> ・**Distribution binaries + bundled assets (characters / images / audio etc.)**: governed by EULA — **non-commercial / free distribution / personal use only** (sales, paid distribution, public re-distribution, and re-releasing modified versions under the "kurotan" name are prohibited)
> See [LICENSE](LICENSE) and [EULA.md](EULA.md) for details. Support is welcome via ⭐ Star and mentions.
...
4. Accept EULA -> confirm install location -> "Install"
   - **Accepting the EULA at install time also constitutes consent for the first-launch hook auto-registration** (see [EULA §3](EULA.md))
...
**Source code forks are welcome under the MIT License.** However, distribution binaries and bundled character assets are governed by [EULA.md](EULA.md) (non-commercial / personal use only; no sales, paid distribution, ad integration, or in-app purchases). When forking, please credit the original repo.
...
This repository uses a **two-layer license structure**.

| Target | License | Permissions |
|---|---|---|
| **Source code** (`src/`, `scripts/`, build configs etc.) | [MIT License](LICENSE) | fork / modify / embed into other apps / redistribute — all permitted within MIT scope |
| **Distribution binaries + bundled assets** (installer EXE, character images, audio, UI assets etc.) | [EULA.md](EULA.md) (additionally applied on top of MIT) | non-commercial / free distribution / personal use only. Sales / paid distribution / public store redistribution / releasing modified versions under the "kurotan" name are prohibited |

You may fork the source under MIT and embed it into a separate app; however if the derivative work **reuses kurotan's bundled assets as-is**, the EULA constraints carry over (this can be avoided by replacing character assets with original artwork).

For Anthropic / Claude logo and wordmark handling, see [NOTICE.md](NOTICE.md).
